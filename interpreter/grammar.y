%{
    ...
    enum OPTYPE { OP_SET, OP_ADD, OP_SUB, OP_MUL, OP_DIV, OP_PRINT, OP_IF, OP_WHILE };
    struct E_STRUCT {
        enum OPTYPE optype;
        int isArg1Str;
        int isArg1E;
...
void processInstruction(OPERATION * op) {
    ...
    switch(op->optype) {
        ...
        case OP_IF: {
            ...
            std::cout<<"IF: "<<(run ? "true, wykonujemy instrukcje" : "false, nie wykonujemy instrukcji")<<"\n";
            if(run) {
        ...
        case OP_WHILE: {
            ...
            while(run) {
                ...
                std::cout<<"WHILE: "<<(run ? "true, wykonujemy instrukcje" : "false, nie wykonujemy instrukcji")<<"\n";
                if(run) {
                    processInstruction(op->toRunIfTrue);
